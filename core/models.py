...
class Pharmacy(models.Model):
    Pharma_ID = models.CharField(max_length=50, blank=True, null=True)
    Address = models.TextField(max_length=100, blank=True, null=True)
    contact = models.CharField(max_length=10, blank=True, null=True)
    Phara_certi_id = models.CharField(max_length=50, blank=True, null=True)


class Prescription(models.Model):
    doctor_id = models.ForeignKey(Doctor, on_delete=CASCADE)
    patient_id = models.ForeignKey(Patient, on_delete=CASCADE)
    Pharma_ID = models.ForeignKey(Pharmacy, on_delete=CASCADE)
